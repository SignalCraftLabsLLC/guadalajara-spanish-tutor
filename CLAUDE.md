...
| `lessons.json` | Data file for scenario/conversation lessons. Each entry: a situational dialogue (e.g. "En el mercado") drawn from real dictionary words and citing specific `grammar_catalog.md` sections, plus a `vocab_highlight` list and `practice` deep-links into the flashcards/sentence maker apps. One pilot scenario (`mercado`) as of June 29, 2026, awaiting Stan's review before more are drafted. |
| `lecciones.html` | Scenario-lesson viewer, loads `lessons.json`. Single Lecciones card (not one card per scenario) with an internal hash-routed picker → detail view (`#mercado`, etc.). Picker shows a friendly mastery chip per lesson (no Leitner/box jargon) computed from the existing `tapatio_dict_v` + `tapatio_leitner` localStorage caches — no new progress-tracking storage key. Detail view renders dialogue as chat bubbles, a vocab chip list, grammar callouts citing exact `grammar_catalog.md` section numbers, and practice links (plain links into `flashcards_vocabulario.html`/`sentence_maker.html`, no URL pre-filtering support yet — deferred, see Potential Next Steps). Text-only, no audio, per Stan's design call. Matches index.html's Talavera design system. |
...
- **Lesson structure: pilot built 2026-06-29** (`lessons.json` + `lecciones.html`, one scenario — "En el mercado"), awaiting Stan's review of the format before drafting more scenarios (doctor's visit, asking directions, etc.). Once approved: add URL-param category pre-filtering to `flashcards_vocabulario.html`/`sentence_maker.html` so practice links can deep-link straight into a filtered deck instead of plain unfiltered links with manual-selection guidance text.
