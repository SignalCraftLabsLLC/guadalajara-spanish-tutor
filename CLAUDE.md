...
- **Lesson format approved and expanded (Session 30)**: four conversation scenarios live (mercado, medico, direcciones, familia) with Leer/Participar modes and branching choice points. Still open: URL-param category pre-filtering for `flashcards_vocabulario.html`/`sentence_maker.html` so lesson practice links can deep-link into a filtered deck instead of plain links with manual-selection guidance text. Also natural next scenarios: en la taquería 🌵 (Stan deferred it this round), en el camión, de compras (ropa). More lessons could get choice nodes — each currently has exactly one.
